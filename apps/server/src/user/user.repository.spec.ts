...
describe('UserRepository', () => {
  ...
  describe('createWithRole', () => {
    it('should create a user with a role', async () => {
      ...
      // CHECK RESULTS
      expect(createdUser.username).toBe(user.username);
      expect(userRoles[0]?.name).toBe(BaseRoles[BaseRoles.USER]);
    });
  ...
  describe('findAllRoles', () => {
    ...
    it('should return a list of roles', async () => {
      ...
      // CHECK RESULTS
      expect(roles[0].id).toBe(BaseRoles.ADMIN);
      expect(roles[0].name).toBe(BaseRoles[BaseRoles.ADMIN]);
    });
  });
});
